...
def test_timestep():
    ...
    simulation.update_drone(drone, packages, dt, charging_pads)
...
def test_deadline_expires():
    ...
    simulation.run_simulation_step(drone, packages, 1.0, 11.0, charging_pads)
    
    assert packages[1].assigned_drone == -1, "Package should be marked as impossible/expired"
...
def test_charging_logic():
    ...
    base_x, base_y = simulation.BASE
    for i in range(1, 5):
        drones[i] = drone_sim.DroneState(id=i, x=base_x, y=base_y, battery=20.0, battery_capacity=95.0, status="RETURNING", target=(base_x, base_y))
